...
function Admin() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (currentScheme.type === 'form' && !currentScheme.downloadUrl) return alert("PDF is mandatory for forms.");
        ...
        try {
            if (isEditing) {
                await supabase.from('schemes').update(schemeData).eq('id', currentScheme.id);
                alert("Item updated!");
                setIsEditing(false);
            } else {
                await supabase.from('schemes').insert([schemeData]);
                alert("Item added!");
            }
